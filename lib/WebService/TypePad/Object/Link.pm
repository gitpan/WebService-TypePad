package WebService::TypePad::Object::Link;
use strict;
use warnings;
...
sub target_url {
    my $self = shift;
    if (@_) {
        $self->{data}{targetUrl} = WebService::TypePad::Util::Coerce::coerce_string_in($_[0]);
        return $_[0];
    }
    else {
        return $self->{data}{targetUrl};
    }
}
...
WebService::TypePad::Object::Link - Perl representation of TypePad's Link object type

=head1 SYNOPSIS

    use WebService::TypePad::Object::Link;
    my $link = WebService::TypePad::Object::Link->new();

=head1 DESCRIPTION

This is a Perl representation of TypePad's Link object type.
For more information about this type and its parameters, see L<the documentation on TypePad's developer site|http://www.typepad.com/services/apidocs/objecttypes/Link>.
...
=head2 $link->is_favorite_for_current_user
...
=head2 $link->target_url

The URL that is the target of this link.

Returns a single C<string> value.

=head2 $link->text_format
...
=item * L<http://www.typepad.com/services/apidocs/objecttypes/Link>
